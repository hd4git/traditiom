...
## Options
data_dir = "/rds/general/user/hdhiman/ephemeral/T47D_targetedPanel/data/results/final/rds"
out_dat_dir = "/rds/general/user/hdhiman/ephemeral/T47D_targetedPanel/data/results/final/filtered_vcf_data"
out_dat_dir_all = "/rds/general/user/hdhiman/ephemeral/T47D_targetedPanel/data/results/final/filtered_vcf_data_raw"
...
# 1: info fields with no header: p



setwd("/rds/general/user/hdhiman/home/wgs/timon/T47D_patypus/")
data_dir = "filtered_vcf_data"
vcf_files = data_dir %>% 
  list.files("[.]rds$", full.names = TRUE) %>% 
  magrittr::set_names(basename(.))
...
for (i in seq_along(vcf_files)) {
  ...
  # mat <- cbind(loc, mat)
  write.table(rownames(mat), file = paste("filtered_vcf_txt", paste(paste("T47D_platypus", nm, sep="_"), "txt", sep="."), sep="/"), sep="\t", quote=F, row.names=F, col.names=T)
}  

data_dir = "/rds/general/user/hdhiman/home/wgs/timon/T47D_patypus/filtered_vcf_data"
vcf_files = data_dir %>% 
  list.files("[.]rds$", full.names = TRUE) %>% 
  magrittr::set_names(basename(.))

setwd("~/wgs/timon/T47D_patypus/filtered_vcf_files/")
for (i in seq_along(vcf_files)) {
  nm <- str_split(vcf_files[i], "\\.", simplify=TRUE)[,2]
  data <- readRDS(vcf_files[i])
  writeVcf(data, file = paste(paste("T47D_platypus", nm, sep="_"),"vcf", sep="."))
}

cd ~/wgs/timon/T47D_patypus/filtered_vcf_files/
cat T47D_platypus_chr1_vep.vcf > ../T47D_platypus.vcf
while read f;
do 
grep -v "#" $f >> ../T47D_platypus.vcf
