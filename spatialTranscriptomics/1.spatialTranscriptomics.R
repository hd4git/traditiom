...
# load data
patientData <- readNanoStringGeoMxSet(dccFiles = DCCFiles,
                           pkcFiles = PKCFiles,
...
setwd("~/spatialTranscriptomics/")
saveRDS(patientData, "rds/data.rds")

patientData<-readRDS("rds/data.rds")
data <- patientData
dim(data)
# Features  Samples
#    18815      173
...
# pData(patientData)[["NTC"]] = rtsCounts[match(rownames(pData(patientData)), rtsCounts$L1),]$value
...
## Sample Overview
count_mat <- count(pData(patientData), slideName, sampleName, segment, aoi)
...
# Shift counts to one
patientData <- shiftCountsOne(patientData, useDALogic = TRUE)
...
patientData <- setSegmentQCFlags(patientData, qcCutoffs = QC_params)        
pData(patientData)[is.na(pData(patientData))]=0

# Collate QC Results
QCResults <- protocolData(patientData)[["QCFlags"]]
...
negativeGeoMeans <- 
    esBy(negativeControlSubset(patientData), 
         GROUP = "Module", 
         FUN = function(x) { 
             assayDataApply(x, MARGIN = 2, FUN = ngeoMean, elt = "exprs") 
         }) 
protocolData(patientData)[["NegGeoMean"]] <- negativeGeoMeans

# explicitly copy the Negative geoMeans from sData to pData
negCols <- paste0("NegGeoMean_", modules)
pData(patientData)[, negCols] <- sData(patientData)[["NegGeoMean"]]

# sData(patientData)$segment<-factor(sData(patientData)$segment, levels=c("CK+","CD45+", "Rest", "Control"))
pdf("plots/QC_histogram.pdf")
QC_histogram(sData(patientData), "Trimmed (%)", col_by, 80)
QC_histogram(sData(patientData), "Stitched (%)", col_by, 80)
QC_histogram(sData(patientData), "Aligned (%)", col_by, 80)
QC_histogram(sData(patientData), "Saturated (%)", col_by, 50) +
    labs(title = "Sequencing Saturation (%)",
         x = "Sequencing Saturation (%)")
QC_histogram(sData(patientData), "area", col_by, 5000, scale_trans = "log10")
# QC_histogram(sData(patientData), "nuclei", col_by, 20)

for(ann in negCols) {
    plt <- QC_histogram(pData(patientData), ann, col_by, 2, scale_trans = "log10")
    print(plt)
}
dev.off()
...
pData(patientData) <- pData(patientData)[, !colnames(pData(patientData)) %in% negCols]

# show all NTC values, Freq = # of Segments with a given NTC count:
ntcData <- patientData[, pData(patientData)$slideName == "No Template Control"]
...
patientData <- patientData[, QCResults$QCStatus == "PASS"]
...
# Features  Samples
#    18815      107
qcPassData <- patientData
filteredData <- patientData[!fData(patientData)$RTS_ID %in% highRTS,]
...
patientData <- setBioProbeQCFlags(patientData, 
                               qcCutoffs = list(minProbeRatio = 0.1,
                                                percentFailGrubbs = 20), 
                               removeLocalOutliers = TRUE)

ProbeQCResults <- fData(patientData)[["QCFlags"]]
...
ProbeQCPassed <- 
    subset(patientData, 
           fData(patientData)[["QCFlags"]][,c("LowProbeRatio")] == FALSE &
               fData(patientData)[["QCFlags"]][,c("GlobalGrubbsOutlier")] == FALSE)
dim(ProbeQCPassed)
# Features  Samples
#    18764      107
patientData <- ProbeQCPassed 

# Check how many unique targets the object has
length(unique(featureData(patientData)[["TargetName"]]))
#> [1] 18635

# patientData <- patientData[rownames(pData(patientData)) %in% c("DSP-1001660009084-H-A01.dcc", "DSP-1001660009086-G-A01.dcc"), ]

# collapse to targets
target_patientData <- aggregateCounts(patientData)
dim(target_patientData)
#> Features  Samples 
#>    18635      107
exprs(target_patientData)[1:5, 1:2]
...
LOQ <- data.frame(row.names = colnames(target_patientData))
for(module in modules) {
    vars <- paste0(c("NegGeoMean_", "NegGeoSD_"),
                   module)
    if(all(vars[1:2] %in% colnames(pData(target_patientData)))) {
        LOQ[, module] <-
            pmax(minLOQ,
                 pData(target_patientData)[, vars[1]] * 
                     pData(target_patientData)[, vars[2]] ^ cutoff)
    }
}
pData(target_patientData)$LOQ <- LOQ

## filtering based on LOQ
LOQ_Mat <- c()
for(module in modules) {
    ind <- fData(target_patientData)$Module == module
    Mat_i <- t(esApply(target_patientData[ind, ], MARGIN = 1,
                       FUN = function(x) {
                           x > LOQ[, module]
                       }))
...
LOQ_Mat <- LOQ_Mat[fData(target_patientData)$TargetName, ]


## Segment gene detection
# Save detection rate information to pheno data
pData(target_patientData)$GenesDetected <- 
    colSums(LOQ_Mat, na.rm = TRUE)
pData(target_patientData)$GeneDetectionRate <-
    pData(target_patientData)$GenesDetected / nrow(target_patientData)

# Determine detection thresholds: 1%, 5%, 10%, 15%, >15%
pData(target_patientData)$DetectionThreshold <- 
    cut(pData(target_patientData)$GeneDetectionRate,
        breaks = c(0, 0.01, 0.05, 0.1, 0.15, 1),
        labels = c("<1%", "1-5%", "5-10%", "10-15%", ">15%"))

# stacked bar plot of different cut points (1%, 5%, 10%, 15%)
pdf("plots/geneDetectionRate0.pdf")
ggplot(pData(target_patientData),
...
kable(table(pData(target_patientData)$DetectionThreshold,
            pData(target_patientData)$sampleName))
...
target_patientData <-
    target_patientData[, pData(target_patientData)$GeneDetectionRate >= .05]
...
count_mat2 <- count(pData(target_patientData), slideName, sampleName, segment,  aoi)
...
LOQ_Mat <- LOQ_Mat[, colnames(target_patientData)]
fData(target_patientData)$DetectedSegments <- rowSums(LOQ_Mat, na.rm = TRUE)
fData(target_patientData)$DetectionRate <-
    fData(target_patientData)$DetectedSegments / nrow(pData(target_patientData))
...
# goi_df <- data.frame(
#     Gene = goi,
#     Number = fData(target_patientData)[goi, "DetectedSegments"],
#     DetectionRate = percent(fData(target_patientData)[goi, "DetectionRate"]))
# Plot detection rate:
plot_detect <- data.frame(Freq = c(1, 5, 10, 20, 30, 50))
plot_detect$Number <-
    unlist(lapply(c(0.01, 0.05, 0.1, 0.2, 0.3, 0.5),
                  function(x) {sum(fData(target_patientData)$DetectionRate >= x)}))
plot_detect$Rate <- plot_detect$Number / nrow(fData(target_patientData))
...
negativeProbefData <- subset(fData(target_patientData), CodeClass == "Negative")
neg_probes <- unique(negativeProbefData$TargetName)
target_patientData <- 
    target_patientData[fData(target_patientData)$DetectionRate >= 0.05 |
                        fData(target_patientData)$TargetName %in% neg_probes, ]
dim(target_patientData)
#> Features  Samples 
#>    6065      85

# retain only detected genes of interest
# goi <- goi[goi %in% rownames(target_patientData)]
...
Stat_data <- 
    data.frame(row.names = colnames(exprs(target_patientData)),
               Segment = colnames(exprs(target_patientData)),
               Annotation = pData(target_patientData)[, ann_of_interest],
               Q3 = unlist(apply(exprs(target_patientData), 2,
                                 quantile, 0.75, na.rm = TRUE)),
               NegProbe = exprs(target_patientData)[neg_probes, ])
...
target_patientData <- normalize(target_patientData ,
                             norm_method = "quant", 
                             desiredQuantile = .75,
                             toElt = "q_norm")
...
boxplot(exprs(target_patientData)[,1:10],
        col = "#9EDAE5", main = "Raw Counts",
        log = "y", names = 1:10, xlab = "Segment",
        ylab = "Counts, Raw")
boxplot(assayDataElement(target_patientData[,1:10], elt = "q_norm"),
        col = "#2CA02C", main = "Q3 Norm Counts",
        log = "y", names = 1:10, xlab = "Segment",
        ylab = "Counts, Q3 Normalized")
boxplot(assayDataElement(target_patientData[,1:10], elt = "neg_norm"),
...
# run UMAP
umap_out <-
    umap(t(log2(assayDataElement(target_patientData , elt = "q_norm"))),  
         config = custom_umap)
pData(target_patientData)[, c("UMAP1", "UMAP2")] <- umap_out$layout[, c(1,2)]
set.seed(42) # set the seed for tSNE as well
tsne_out <-
    Rtsne(t(log2(assayDataElement(target_patientData , elt = "q_norm"))),
          perplexity = ncol(target_patientData)*.15)
pData(target_patientData)[, c("tSNE1", "tSNE2")] <- tsne_out$Y[, c(1,2)]


saveRDS(target_patientData, "rds/target_patientData.rds")
...
ggplot(pData(target_patientData)[pData(target_patientData)$segment == "CK+",],
...
CV_dat <- assayDataApply(target_patientData,
...
allGenes <- fData(target_patientData)$TargetName
...
pheatmap(assayDataElement(target_patientData[sanityChecks, ], elt = "log_q"),
         scale = "row", 
         show_rownames = TRUE, 
         show_colnames = FALSE,
         cutree_cols = 2,
         ...
         annotation_col = pData(target_patientData)[, c("segment", "sampleName")], 
...
sanityCheckExp<-as.data.frame(assayDataElement(target_patientData[sanityChecks, ], elt = "log_q"))
pData(target_patientData) <- merge(pData(target_patientData), t(sanityCheckExp), by=0, all=TRUE)
rownames(pData(target_patientData)) <- pData(target_patientData)[,1]

require("MatrixGenerics")
require("ggpubr")
require("viridis")
allExp <- as.data.frame(assayDataElement(target_patientData, elt = "log_q"))
...
data <- as.data.frame(pData(target_patientData))
...
dev.off()
treatment <- ggplot(pData(target_patientData)[pData(target_patientData)$segment == "CK+",],
...
expMat <- as.data.frame(assayDataElement(target_patientData, elt = "log_q"))
